...
overview: 对 AutoCMEX 项目进行系统性结构优化，解决类职责越界、DI 不统一、UI 耦合过紧、代码重复等问题，按六个阶段分步实施，提升项目可维护性。
todos:
  - id: core-cleanup
  ...
  - id: service-abstraction
    ...
    dependencies:
      - core-cleanup
  - id: ui-extraction
    content: 使用 [skill:task-commander] 并行执行 UI 层职责分离：GuessingPanel 移除 EscapeCsv/改用 DI/接入 ObservableCollection、LogPanel 移除 EscapeBbcode/分离 LogConfigPanel/改用 [Dependency] 获取服务、SettingsPanel 移除 CopyPluginDir/改用 AiServiceFactory、WebSocketPanel 替换为 Godot Timer/改用 [Dependency] 获取服务
    status: completed
    dependencies:
      - service-abstraction
  - id: architecture-unify
    content: 使用 [skill:task-commander] 执行架构统一：提取 WebSocketInitializer 消除 MainWindow 重复初始化、统一 DI 为 IProvide/[Dependency]+DependOn 配套模式（消除 AppLogs.Current/SetServer 非标准路径）、移除 BuildClientUrl 残留
    status: completed
    dependencies:
      - ui-extraction
  - id: update-tests
    content: 使用 [skill:tdd] 和 [skill:csharp-test-granularity-standard] 更新全部测试：覆盖新增工具类、抽象层、重构后的服务方法，确保测试通过且覆盖率不低于80%
    status: completed
    dependencies:
      - architecture-unify
  - id: update-docs
...
- **InMemoryLogWriter**：将 `ParseFormatted` 字符串解析逻辑提取到 `LogEntry.FromFormattedString` 静态工厂方法
...
- **GuessProcessingService**：合并 `ProcessManualAsync`/`ProcessManagedAsync` 为单一 `ProcessAsync`，filterMode 从设置读取，移除 currentBoss 参数，Error 由调用方各自处理
- **ConnectionManager**：统一双重发送路径为单一异步路径
- **CsvImporter/ExcelImporter**：提取 `IImporter` 接口 + `ImporterFactory` 工厂模式

### UI 层职责分离（12项）

- **GuessingPanel**：提取 `EscapeCsv` 到 `StringEscapeHelper`；`OnProcessGuess` 改用 DI 注入服务
- **GuessingPanel 丢包 UI**：将 `SetupDroppedUI()` 中动态创建的丢包重试 UI 迁移到 `GuessingPanel.tscn` 场景中，使用 `[Node]` 属性引用
- **LogPanel**：提取 `EscapeBbcode` 到 `StringEscapeHelper`；分离日志配置管理为独立 `LogConfigPanel`
- **SettingsPanel**：提取 `CopyPluginDir` 到 `PluginInstaller` 服务；`TestModelConnection` 改用 `AiServiceFactory.CreateService`
- **MainWindow**：`BuildClientUrl` 移到 `WebSocketClient`；剥离 WebSocket 初始化到 `WebSocketInitializer`；消除 `OnReady` 与 `RestartWebSocket` 重复代码
- **MainWindow 子面板初始化**：将 `PreloadPanels()`、`SetupLogPanel()`、`SetupWebSocketPanel()` 中动态加载的子面板迁移到 `MainWindow.tscn` 场景中实例化，MainWindow 仅通过 `[Node]` 属性引用
...
- **MainWindow God Object 分解**：剥离 WebSocket 初始化、URL 构造等非 DI 编排职责到独立类（`WebSocketInitializer`），MainWindow 保留 DI 提供方 + UI 窗口管理职责
...
- **语言**：C# (.NET 8)
- **引擎**：Godot 4.x (Mono)
- **DI 框架**：Chickensoft.AutoInject（`IProvide<T>` / `[Dependency]` + `DependOn<T>()`）
...
采用**渐进式重构**策略，按依赖关系自底向上分 5 个阶段执行：核心层清理 → 服务抽象层提取 → UI 层职责分离 → 架构统一 → 测试与文档更新。每阶段独立可验证，降低回归风险。
...
7. **ObservableCollection 集成**：使用 `ObservableCollection<T>` 替代 `List<T>` 作为 DataManager 的猜词列表容器，GuessingPanel 绑定 `CollectionChanged` 事件自动刷新 UI，消除手动 `RefreshGuessList()` 调用。
...
graph TD
    subgraph "DI 提供方"
        MW[MainWindow : IProvide] --> DM[DataManager]
        MW --> LS[LogService]
...
│   ├── guessing/
│   │   └── GuessingPanel.cs        # [MODIFY] 移除 EscapeCsv，OnProcessGuess 改用 DI，接入 ObservableCollection
│   ├── logging/
│   │   ├── LogPanel.cs             # [MODIFY] 移除 EscapeBbcode，改用 [Dependency] 获取服务，分离配置管理
│   │   └── LogConfigPanel.cs       # [NEW] 日志配置独立面板
│   ├── settings/
│   │   └── SettingsPanel.cs        # [MODIFY] 移除 CopyPluginDir，TestModelConnection 改用 AiServiceFactory
│   └── websocket/
│       └── WebSocketPanel.cs       # [MODIFY] System.Timers.Timer → Godot Timer，改用 [Dependency] 获取服务
├── services/
...
- JSON 深拷贝仅保存配置时调用（非热路径），单次耗时小于1ms，无性能影响
...
- 每阶段独立提交，通过 `dotnet build` + `dotnet test` 验证
- 接口变更优先，实现变更在后，确保编译期捕获错误
- 保留旧方法标记 `[Obsolete]` 过渡一个阶段后再移除
...
- **task-commander**
- 用途：将本重构方案分解为依赖排序的子任务，使用 SubAgent 并行执行独立任务（如 UI 层多个面板的职责分离可并行处理）
- 预期结果：各阶段任务按依赖顺序高效执行，独立模块并行处理，缩短整体重构周期
