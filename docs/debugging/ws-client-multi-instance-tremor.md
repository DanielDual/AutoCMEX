...
心跳用例的判别力偏弱：`StopAsync` 后不再有 ping 这条能守住「停止后不留任务」，但停止时旧写法的心跳同样会随（共享的）令牌退出，所以它抓不住「重连后旧心跳跟着新 socket 继续发」这个真正的动机场景（要抓它得比「重连前后的 ping 频率」，属计时类用例）。本次不动：行为已由「每条连接一个作用域 + socket 快照」在结构上排除，再加计时类断言只会引入不稳定的用例，需要时再单独评估。

停止时若正处于「正在连接」阶段，新实现会补触发一次 `OnClientDisconnected`（旧实现在这条路径上直接 `break`，不触发），状态面板的事件行因此会显示一次「已断开」。这是「连接确实结束了」的如实反映（其唯一订阅方是状态面板的事件行文案，不影响连接列表），故保留而不特判。
